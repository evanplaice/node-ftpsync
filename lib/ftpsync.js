...
var utils = exports.utils = {
  ...
  mkdir: function(dir, callback) {
    ...
    ftp.raw.mkd(dir, function(err, data) {
      ...
      if (sync.log.verbose) {
        sync.log.write('-', dir, 'created successfully');
      }
      callback();
    });
  },
  ...
  rmdir: function(dir, callback) {
    ...
    ftp.raw.rmd(dir, function(err, data) {
      ...
      if (sync.log.verbose) {
        sync.log.write('-', dir, 'deleted successfully');
      }
      callback();
    });
  },
  ...
  upload: function(file, callback) {
    ...
    ftp.put(local, remote, function(err) {
      ...
      if (sync.log.verbose) {
        sync.log.write('-', file, 'uploaded successfully');
      }
      callback(null, file);
    });
  },
  ...
  remove: function(file, callback) {
    ...
    ftp.raw.dele(file, function(err, data) {
      ...
      if (sync.log.verbose) {
        sync.log.write('-', file, 'deleted successfully');
      }
      callback(null, file);
    });
  }
